...
def arbitrage_stats(arbitrage, wallet_amount):
    ...
    total_return = 0 if real_investment == 0 else 100 * profit / real_investment
...
def get_sorted_arb_stats(arbitrages, wallet_amount):
...
parquet_filenames = sys.argv[1:]

num_files = len(parquet_filenames)

print(f"{num_files=}", file=sys.stderr)

file_num = 0

for parquet_filename in parquet_filenames:
    print(f"{file_num=}: ", end="", file=sys.stderr)
    file_num += 1

    start = datetime.datetime.now()
    get_best_arb(parquet_filename, file_num)
    end = datetime.datetime.now()
    time_taken = end-start

    time_left = (num_files - file_num) * time_taken
    print(time_left, file=sys.stderr)